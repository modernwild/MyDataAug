...
def get_dataset(conf):
    dataset_path = r'D:\Datasets\CUB_200_2011'  # 默认路径
    conf['num_class'] = 200

    if conf and 'dataset_path' in conf:
        dataset_path = conf.dataset_path

    transform_train, transform_test = get_cub_transform(conf)

    ds_train = ImageLoader(dataset_path, train=True, transform=transform_train)
    ds_test = ImageLoader(dataset_path, train=False, transform=transform_test)
